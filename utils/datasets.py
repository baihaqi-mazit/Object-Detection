...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416, class_names=[]):
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            # ISSUE: Image width and height interchange for whatever reason =.=
            print('self.__annotations[item]', self.__annotations[item])
    ...
    def __parse_annotation(self, annotation):
        ...
        assert len(bboxes) != 0, 'Bboxes empty' + img_path

        if cfg.AUGMENT['DILATION']: img, bboxes = dataAug.Dilation()(np.copy(img), np.copy(bboxes))
        if cfg.AUGMENT['SMUDGE']: img, bboxes = dataAug.Smudge()(np.copy(img), np.copy(bboxes))
        if cfg.AUGMENT['RANDOM_CHANGE_COLOR']: img, bboxes = dataAug.RandomChangeColor()(np.copy(img), np.copy(bboxes))
        if cfg.AUGMENT['RANDOM_HORIZONTAL_FLIP']: img, bboxes = dataAug.RandomHorizontalFilp()(np.copy(img), np.copy(bboxes))
        
        # try:
        if cfg.AUGMENT['RANDOM_AFFINE']: img, bboxes = dataAug.RandomAffine()(np.copy(img), np.copy(bboxes))
        if cfg.AUGMENT['RANDOM_CROP']: img, bboxes = dataAug.RandomCrop()(np.copy(img), np.copy(bboxes))
        if cfg.AUGMENT['RANDOM_ROTATE']: img, bboxes = dataAug.RandomRotate(cfg.AUGMENT['RANDOM_ROTATE_RANGE'])(np.copy(img), np.copy(bboxes))
        # except:
        #     print(img, bboxes)
